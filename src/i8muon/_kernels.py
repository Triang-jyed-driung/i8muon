...
def _to_bq_transpose_out(
    ...
    """Fused _to_bq + transpose: writes A_bq as (N, M), A_scale as (nd, md)."""
    assert dtype_out in ['float8_e4m3fn', 'float8_e4m3', 'int8']
    md = T.ceildiv(M, BLOCK_Q)
    nd = T.ceildiv(N, BLOCK_Q)
    DTYPE_MAX = 448 if 'float8_e4m3' in dtype_out else 127 
    ...
    def _to_bq_transpose_out_(
        A: T.Tensor((M, N), dtype), # type: ignore
        A_sumsq_or_norm: T.Tensor((1,), dtype2), # type: ignore
        A_bq: T.Tensor((N, M), dtype_out),  # type: ignore  NOTE: transposed
        A_scale: T.Tensor((nd, md), T.float32) # type: ignore  NOTE: transposed
    ):
        with T.Kernel(nd, md, threads=threads) as (pid_n, pid_m):
            A_scale_1 = T.alloc_var(T.float32)
            A_scale_2 = T.alloc_reducer((1,), T.float32, op="max", replication="all")
            ...
            A_scale_1 = DTYPE_MAX / A_scale_2[0]
            if (T.get_lane_idx() == 0 and T.get_warp_idx() == 0):
                A_scale[pid_n, pid_m] = 1 / A_scale_1
            for i, j in T.Parallel(BLOCK_Q, BLOCK_Q):
                A_local[i, j] = A_local[i, j] * A_scale_1
            # Direct transposed write (BLOCK_Q=128 is square, coalesced)
            for j, i in T.Parallel(BLOCK_Q, BLOCK_Q):
                A_bq[pid_n * BLOCK_Q + j, pid_m * BLOCK_Q + i] = T.cast(A_local[i, j], dtype_out)
...
def _typeii_typei_final_bq_transpose_out(
    ...
    """Fused _typeii_typei_final_bq + transpose: writes C as (N, M)."""
    md = T.ceildiv(M, BLOCK_Q)
    nd = T.ceildiv(N, BLOCK_Q)
    accum_dtype = 'float32' if 'float8_e4m3' in dtype else 'int32'
    ...
    def _typeii_typei_final_bq_transpose_out_(
        ...
        B:       T.Tensor((M, N), dtype), # type: ignore
        B_scale: T.Tensor((md, nd), T.float32), # type: ignore
        C:       T.Tensor((N, M), dtype2), # type: ignore  NOTE: transposed
    ):
        with T.Kernel(nd, md, threads=threads) as (pid_n, pid_m):
            A_shared = T.alloc_shared((BLOCK_Q, BLOCK_Q), dtype)
            B_shared = T.alloc_shared((BLOCK_Q, BLOCK_Q), dtype)
            ...
            # Direct transposed write (BLOCK_Q=128 is square, coalesced)
            for j, i in T.Parallel(BLOCK_Q, BLOCK_Q):
                C[pid_n * BLOCK_Q + j, pid_m * BLOCK_Q + i] = C_float[i, j]

    return _typeii_typei_final_bq_transpose_out_


####################### Fused transpose-out variants ########################################
# These kernels eliminate the explicit .mT.contiguous() calls by writing output
def _scale_int8_transpose_out(
    ...
    def _scale_int8_transpose_out_(
        ...
        A_max: T.Tensor((1,), T.float32), # type: ignore
        A_sumsq_or_norm: T.Tensor((1,), dtype2), # type: ignore
        A_int8: T.Tensor((N, M), T.int8),  # type: ignore  NOTE: transposed shape
        A_scale: T.Tensor((1,), T.float32) # type: ignore
    ):
        with T.Kernel(T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (pid_n, pid_m):
            if pid_n == 0 and pid_m == 0 and T.get_lane_idx() == 0 and T.get_warp_idx() == 0:
            ...
            for i, j in T.Parallel(BLOCK_M, BLOCK_N):
                C_local_i8[i, j] = T.cast(T.round((127.0 / A_max[0]) * T.cast(C_local[i, j], T.float32)), T.int8)
            # Fused transpose via shared memory — avoids global mem round-trip
            ...
            T.copy(A_shared_T, A_int8[pid_n * BLOCK_N, pid_m * BLOCK_M])
...
def _typeii_typei_int8_transpose_out(
    ...
    def _typeii_typei_int8_transpose_out_(
        ...
        B: T.Tensor((M, N), T.int8), # type: ignore
        B_scale: T.Tensor((1,), T.float32), # type: ignore
        C: T.Tensor((N, M), T.float32), # type: ignore  NOTE: transposed shape
        C_max: T.Tensor((1,), T.float32), # type: ignore
    ):
        with T.Kernel(T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (pid_n, pid_m):
            A_shared = T.alloc_shared((BLOCK_M, BLOCK_K), T.int8)
            ...
            for k in T.Pipelined(T.ceildiv(M, BLOCK_K), num_stages=num_stages):
                ...
                T.copy(B[k * BLOCK_K, pid_n * BLOCK_N], B_shared) 
                T.gemm(A_shared, B_shared, C_local)
            # Free shared memory used by GEMM before computing epilogue
            del A_shared
            del B_shared
            T.copy(A_diag[pid_m * BLOCK_M], row_shared)
            B8 = T.alloc_fragment((BLOCK_M, BLOCK_N), T.int8)
            ...
            for (i, j) in T.Parallel(BLOCK_M, BLOCK_N):
                ...
                val = T.abs(C_float[i, j])
                Cmax_reducer[0] = T.max(val, Cmax_reducer[0])
            # Direct transposed write: (j,i) loop with i inner → coalesced (stride-1 col access)
            for j, i in T.Parallel(BLOCK_N, BLOCK_M):
                C[pid_n * BLOCK_N + j, pid_m * BLOCK_M + i] = C_float[i, j]
            T.finalize_reducer(Cmax_reducer)
            if T.get_lane_idx() == 0 and T.get_warp_idx() == 0:
                T.atomic_max(C_max[0], Cmax_reducer[0])
...
def _to_prec_transpose_out(
    ...
    def _to_prec_transpose_out_(
        A: T.Tensor((M, N), dtype), # type: ignore
        A_sumsq_or_norm: T.Tensor((1,), dtype2), # type: ignore
        A_prec: T.Tensor((N, M), dtype_out),  # type: ignore  NOTE: transposed shape
    ):
        with T.Kernel(T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (pid_n, pid_m):
            A_scale = T.alloc_var(T.float32)
            A_scale = 1.0 / T.max(
            ...
            for i, j in T.Parallel(BLOCK_M, BLOCK_N):
                A_local[i, j] *= A_scale
            # Fused transpose via shared memory
            ...
            T.transpose(A_shared, A_shared_T)
            T.copy(A_shared_T, A_prec[pid_n * BLOCK_N, pid_m * BLOCK_M])
...
def _ab_prec_transpose_out(M: int, N: int, K: int,
           ...
           dtype: str = 'float16', accum_dtype: str = 'float32'):
    @T.prim_func
    def _ab_prec_transpose_out_(A: T.Tensor((M, K), dtype),  # type: ignore
               B: T.Tensor((K, N), dtype),  # type: ignore
               C: T.Tensor((N, M), dtype)): # type: ignore  NOTE: transposed shape
        with T.Kernel(T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (bx, by):
            A_s = T.alloc_shared((BLOCK_M, BLOCK_K), dtype)
            B_s = T.alloc_shared((BLOCK_K, BLOCK_N), dtype)
            C_f = T.alloc_fragment((BLOCK_M, BLOCK_N), accum_dtype)
            ...
            T.copy(C_shared_T, C[bx * BLOCK_N, by * BLOCK_M])
