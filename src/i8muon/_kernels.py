...
def _sumsq_maxabs(
    ...
    def _sumsq_maxabs_(
        ...
        A_max: T.Tensor((B,), T.float32),
        A_sumsq_or_norm: T.Tensor((B,), T.float32)
    ):
        with T.Kernel(B, T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (pid_b, pid_n, pid_m):
            C_local = T.alloc_fragment((BLOCK_M, BLOCK_N), dtype)
            T.copy(A[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N], C_local)
            ...
            if T.get_lane_idx() == 0 and T.get_warp_idx() == 0:
                T.atomic_max(A_max[pid_b], Cmax_reducer[0])
                T.atomic_add(A_sumsq_or_norm[pid_b], Csquare_reducer[0])
...
def _scale_int8(
    ...
    def _scale_int8_(
        A: T.Tensor((B, M, N), dtype),
        A_max: T.Tensor((B,), T.float32),
        A_sumsq_or_norm: T.Tensor((B,), dtype2),
        A_int8: T.Tensor((B, M, N), T.int8),
        A_scale: T.Tensor((B,), T.float32)
    ):
        with T.Kernel(B, T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (pid_b, pid_n, pid_m):
            if pid_n == 0 and pid_m == 0 and T.get_lane_idx() == 0 and T.get_warp_idx() == 0:
                A_scale[pid_b] = A_max[pid_b] / (127.0 * T.max(
                        T.cast(A_sumsq_or_norm[pid_b], T.float32) 
                        if use_norm else 
                        T.sqrt(T.cast(A_sumsq_or_norm[pid_b], T.float32)), 
                        eps
                    )
                )
            C_local = T.alloc_fragment((BLOCK_M, BLOCK_N), dtype)
            ...
            T.copy(A[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N], C_local)
            for i, j in T.Parallel(BLOCK_M, BLOCK_N):
                C_local_i8[i, j] = T.cast(T.round((127.0 / A_max[pid_b]) * T.cast(C_local[i, j], T.float32)), T.int8)
            T.copy(C_local_i8, A_int8[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N])
    return _scale_int8_
...
def _aat_int8_max(
    ...
    def _aat_int8_max_(
        ...
        C: T.Tensor((B, M, M), accum_dtype),
        C_max: T.Tensor((B,), accum_dtype)
    ):
        with T.Kernel(B, total_blocks, threads=threads) as (pid_b, pid):
...
def _int32_compl_symm_int8(
    ...
    def _int32_compl_symm_int8_(
        A32_lowtri: T.Tensor((B, M, M), T.int32),
        A_max: T.Tensor((B,), T.int32),
        orig_scale: T.Tensor((B,), T.float32),
        A8: T.Tensor((B, M, M), T.int8),
        A_scale: T.Tensor((B,), T.float32),
        A_diag: T.Tensor((B, M), T.float32),
    ):
        with T.Kernel(B, total_blocks, threads=threads) as (pid_b, pid):
            T.assume(BLOCK_M % BLOCK_N == 0)
            base_pid = pid // R
            ...
            scale_A = 127.0 / T.cast(A_max[pid_b], T.float32)
            ...
            T.copy(A32_lowtri[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N], A32_frag)
            
            if (pid_n + 1) * BLOCK_N <= pid_m * BLOCK_M:
                for (i, j) in T.Parallel(BLOCK_M, BLOCK_N):
                    A8_frag[i,j] = T.cast(T.round(T.cast(A32_frag[i,j], T.float32) * scale_A), T.int8)
                T.copy(A8_frag, A8_shared)
                T.copy(A8_shared, A8[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N])
                T.transpose(A8_shared, A8_shared_T)
                T.copy(A8_shared_T, A8[pid_b, pid_n * BLOCK_N, pid_m * BLOCK_M])
            else:
                AA_scale = T.alloc_var(T.float32)
                AA_scale = orig_scale[pid_b]
                AA_scale *= AA_scale
                ...
                for (i, j) in T.Parallel(BLOCK_M, BLOCK_N):
                    ...
                    col = pid_n * BLOCK_N + j
                    A8_frag[i,j] = T.cast(
                        T.if_then_else(row == col, 0, T.round(T.cast(A32_frag[i,j], T.float32) * scale_A)),
                        T.int8
                    )
                    if row == col:
                        A_diag[pid_b, row] = T.cast(A32_frag[i,j], T.float32) * AA_scale
                T.copy(A8_frag, A8[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N])
                if pid_n == 0 and pid_m == 0 and T.get_lane_idx() == 0 and T.get_warp_idx() == 0:
                    A_scale[pid_b] = AA_scale / scale_A
...
def _typeii_int8_sq(
    ...
    def _typeii_int8_sq_(
        A: T.Tensor((B, M, M), T.int8),
        A_scale: T.Tensor((B,), T.float32),
        A_diag: T.Tensor((B, M), T.float32),
        C: T.Tensor((B, M, M), T.float32),
        C_max: T.Tensor((B,), T.float32),
        ALPHA: T.float32,
        BETA: T.float32
    ):
        with T.Kernel(B, total_blocks, threads=threads) as (pid_b, pid):
            ...
            for k in T.Pipelined(T.ceildiv(M, BLOCK_K), num_stages=num_stages):
                T.copy(A[pid_b, pid_m * BLOCK_M, k * BLOCK_K], A_shared)
                T.copy(A[pid_b, k * BLOCK_K, pid_n * BLOCK_N], B_shared) 
                T.gemm(A_shared, B_shared, C_local)
            T.copy(A_diag[pid_b, pid_m * BLOCK_M], row_shared)
            T.copy(A_diag[pid_b, pid_n * BLOCK_N], col_shared)
            ...
            A_var *= A_var
            A_var *= BETA
            T.copy(A[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N], A8)
            Cmax_reducer = T.alloc_reducer((1,), T.float32, op='max', replication="all")
            T.fill(Cmax_reducer, 0)
            for (i, j) in T.Parallel(BLOCK_M, BLOCK_N):
                C_float[i, j] = (
                ...
                val = T.if_then_else(pid_m * BLOCK_M + i == pid_n * BLOCK_N + j, 0, T.abs(C_float[i, j]))
                Cmax_reducer[0] = T.max(val, Cmax_reducer[0])
            T.copy(C_float, C[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N])
            T.finalize_reducer(Cmax_reducer)
            if T.get_lane_idx() == 0 and T.get_warp_idx() == 0:
                T.atomic_max(C_max[pid_b], Cmax_reducer[0])
...
def _float32_compl_symm_int8_quad(
    ...
    def _float32_compl_symm_int8_quad_(
        A32_lowtri: T.Tensor((B, M, M), T.float32),
        A_max: T.Tensor((B,), T.float32),
        orig_diag: T.Tensor((B, M), T.float32),
        A8: T.Tensor((B, M, M), T.int8),
        A_scale: T.Tensor((B,), T.float32),
        A_diag: T.Tensor((B, M), T.float32),
        C0: T.float32,
        ...
        with T.Kernel(B, total_blocks, threads=threads) as (pid_b, pid):
            ...
            if (pid_n + 1) * BLOCK_N <= pid_m * BLOCK_M:
                ...
                for (i, j) in T.Parallel(BLOCK_M, BLOCK_N):
                    ...
                    T.assume(col >= 0)
                    A8_frag[i,j] = T.cast(
                        T.if_then_else(row == col, 0, T.round(A32_frag[i,j] * scale_A)),
                        T.int8
                    )
                    if row == col:
                        A_diag[pid_b, row] = A32_frag[i,j] + (C2 * orig_diag[pid_b, row] + C1) * orig_diag[pid_b, row] + C0
                T.copy(A8_frag, A8[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N])
                if pid_n == 0 and pid_m == 0 and T.get_lane_idx() == 0 and T.get_warp_idx() == 0:
                    A_scale[pid_b] = 1.0 / scale_A
...
def _typeii_int8_ab(
    ...
    def _typeii_int8_ab_(
        ...
        A_scale: T.Tensor((B,), T.float32),
        A_diag: T.Tensor((B, M), T.float32),
        B_mat: T.Tensor((B, M, M), T.int8),
        B_scale: T.Tensor((B,), T.float32),
        B_diag: T.Tensor((B, M), T.float32),
        C: T.Tensor((B, M, M), T.float32),
        C_max: T.Tensor((B,), T.float32),
    ):
        with T.Kernel(B, total_blocks, threads=threads) as (pid_b, pid):
            ...
            for k in T.Pipelined(T.ceildiv(M, BLOCK_K), num_stages=num_stages):
                ...
                T.gemm(A_shared, B_shared, C_local)
            T.copy(A_diag[pid_b, pid_m * BLOCK_M], row_shared)
            T.copy(B_diag[pid_b, pid_n * BLOCK_N], col_shared)
            A8 = T.alloc_fragment((BLOCK_M, BLOCK_N), T.int8)
            ...
            B_var = B_scale[pid_b]
            T.copy(A[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N], A8)
            T.copy(B_mat[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N], B8)
            Cmax_reducer = T.alloc_reducer((1,), T.float32, op='max', replication="all")
            T.fill(Cmax_reducer, 0)
...
def _typeii_typei_int8(
    ...
    def _typeii_typei_int8_(
        ...
        A_scale: T.Tensor((B,), T.float32),
        A_diag: T.Tensor((B, M), T.float32),
        B_mat: T.Tensor((B, M, N), T.int8),
        B_scale: T.Tensor((B,), T.float32),
        C: T.Tensor((B, M, N), T.float32),
        C_max: T.Tensor((B,), T.float32),
    ):
        with T.Kernel(B, T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (pid_b, pid_n, pid_m):
            A_shared = T.alloc_shared((BLOCK_M, BLOCK_K), T.int8)
...
def _float32_to_int8(
    ...
    def _float32_to_int8_(
        C32: T.Tensor((B, M, N), T.float32),
        C_max: T.Tensor((B,), T.float32),
        C: T.Tensor((B, M, N), T.int8),
        C_scale: T.Tensor((B,), T.float32),
    ):
        with T.Kernel(B, T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (pid_b, pid_n, pid_m):
            scale_C = T.alloc_var(T.float32)
            scale_C = 127.0 / C_max[pid_b]
            C32_frag = T.alloc_fragment((BLOCK_M, BLOCK_N), T.float32)
            C_frag = T.alloc_fragment((BLOCK_M, BLOCK_N), T.int8)
            T.copy(C32[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N], C32_frag)
            for (i, j) in T.Parallel(BLOCK_M, BLOCK_N):
                C_frag[i,j] = T.cast(T.round(T.cast(C32_frag[i,j], T.float32) * scale_C), T.int8)
            T.copy(C_frag, C[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N]) 
            if pid_n == 0 and pid_m == 0 and T.get_lane_idx() == 0 and T.get_warp_idx() == 0:
                C_scale[pid_b] = 1.0 / scale_C
...
def _to_prec(
    ...
    def _to_prec_(
        A: T.Tensor((B, M, N), dtype),
        A_sumsq_or_norm: T.Tensor((B,), dtype2),
        A_prec: T.Tensor((B, M, N), dtype_out),
    ):
        with T.Kernel(B, T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (pid_b, pid_n, pid_m):
            A_scale = T.alloc_var(T.float32)
            A_scale = 1.0 / T.max(
                T.cast(A_sumsq_or_norm[pid_b], T.float32) if use_norm else T.sqrt(T.cast(A_sumsq_or_norm[pid_b], T.float32)), 
                eps
            )
            A_local = T.alloc_fragment((BLOCK_M, BLOCK_N), T.float32)
            T.copy(A[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N], A_local)
            for i, j in T.Parallel(BLOCK_M, BLOCK_N):
                A_local[i, j] = A_local[i, j] * A_scale
            T.copy(A_local, A_prec[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N])
    return _to_prec_
...
def _aat_prec(M: int = 4096, K: int = 6144,
    ...
    total_blocks = (U * (U - 1) // 2) * R + T.ceildiv(M, BLOCK_N)
    @T.prim_func
    def _aat_prec_(A: T.Tensor((B, M, K), dtype),
               C: T.Tensor((B, M, M), dtype)):
        with T.Kernel(B, total_blocks, threads=threads) as (pid_b, pid):
            T.assume(BLOCK_M % BLOCK_N == 0)
            base_pid = pid // R
            pid_m = T.alloc_var(T.int32)
            ...
            if (pid_n + 1) * BLOCK_N <= pid_m * BLOCK_M:
                ...
                T.copy(s_shared, C[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N])
                T.transpose(s_shared, t_shared)
                T.copy(t_shared, C[pid_b, pid_n * BLOCK_N, pid_m * BLOCK_M])
            else:
                T.copy(C_local, C[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N])
    return _aat_prec_
...
def _quad_prec(
    ...
    def _quad_prec_(
        ...
        with T.Kernel(B, total_blocks, threads=threads) as (pid_b, pid):
            ...
            s_shared = T.alloc_shared((BLOCK_M, BLOCK_N), dtype)
            t_shared = T.alloc_shared((BLOCK_N, BLOCK_M), dtype)
            T.copy(A[pid_b, pid_m * BLOCK_M, pid_n * BLOCK_N], s_shared)
            if (pid_n + 1) * BLOCK_N <= pid_m * BLOCK_M:
                for i, j in T.Parallel(BLOCK_M, BLOCK_N):
                    s_shared[i, j] = T.cast(
...
def _ab_symm_prec(M: int = 4096,
    ...
    total_blocks = (U * (U - 1) // 2) * R + T.ceildiv(M, BLOCK_N)
    @T.prim_func
    def _ab_symm_prec_(A: T.Tensor((B, M, M), dtype),
               B_mat: T.Tensor((B, M, M), dtype),
               C: T.Tensor((B, M, M), dtype)):
        with T.Kernel(B, total_blocks, threads=threads) as (pid_b, pid):
            T.assume(BLOCK_M % BLOCK_N == 0)
            base_pid = pid // R
            ...
            for k in T.Pipelined(T.ceildiv(M, BLOCK_K), num_stages=num_stages):
                T.copy(A[pid_b, pid_m * BLOCK_M, k * BLOCK_K], A_shared)
                T.copy(B_mat[pid_b, pid_n * BLOCK_N, k * BLOCK_K], B_shared) 
                T.gemm(A_shared, B_shared, C_local, transpose_B=True)
            del A_shared
            del B_shared
...
def _to_bq(
    ...
    def _to_bq_(
        A: T.Tensor((B, M, N), dtype),
        A_sumsq_or_norm: T.Tensor((B,), dtype2),
        A_bq: T.Tensor((B, M, N), dtype_out),
        A_scale: T.Tensor((B, md, nd), T.float32)
    ):
        with T.Kernel(B, nd, md, threads=threads) as (pid_b, pid_n, pid_m):
            ...
            T.copy(A[pid_b, pid_m * BLOCK_Q, pid_n * BLOCK_Q], A_local)
            ...
            if (T.get_lane_idx() == 0 and T.get_warp_idx() == 0):
                A_scale[pid_b, pid_m, pid_n] = 1 / A_scale_1
            for i, j in T.Parallel(BLOCK_Q, BLOCK_Q):
                A_local[i, j] = A_local[i, j] * A_scale_1
            T.copy(A_local, A_bq[pid_b, pid_m * BLOCK_Q, pid_n * BLOCK_Q])
    return _to_bq_
...
def _aat_bq(
    ...
    def _aat_bq_(
        A: T.Tensor((B, M, K), dtype),
        A_scale: T.Tensor((B, md, kd), 'float32'),
        C: T.Tensor((B, M, M), dtype),
        C_scale: T.Tensor((B, md, md), 'float32'),
        C_diag: T.Tensor((B, M), 'float32'),
    ):
        with T.Kernel(B, total_blocks, threads=threads) as (pid_b, pid):
            pid_m = T.alloc_var(T.int32)
            ...
            for k in T.Pipelined(kd, num_stages=num_stages):
                T.clear(C_local)
                T.copy(A[pid_b, pid_m * BLOCK_Q, k * BLOCK_Q], A_shared)
                T.copy(A[pid_b, pid_n * BLOCK_Q, k * BLOCK_Q], B_shared)
                T.gemm(A_shared, B_shared, C_local, transpose_B=True)
                A_scale_1 = A_scale[pid_b, pid_m, k] * A_scale[pid_b, pid_n, k]
                for i, j in T.Parallel(BLOCK_Q, BLOCK_Q):
            ...
            if pid_m == pid_n:
                for i, j in T.Parallel(BLOCK_Q, BLOCK_Q):
                    if i == j:
                        C_diag[pid_b, pid_m * BLOCK_Q + i] = C_float[i, j]
                        C_float[i, j] = 0
                    else:
            ...
            if (T.get_lane_idx() == 0 and T.get_warp_idx() == 0):
                C_scale[pid_b, pid_m, pid_n] = 1 / A_scale_1
                C_scale[pid_b, pid_n, pid_m] = 1 / A_scale_1
            ...
            if pid_m != pid_n:
                ...
                T.copy(C_shared, C[pid_b, pid_m * BLOCK_Q, pid_n * BLOCK_Q])
                T.transpose(C_shared, D_shared)
                T.copy(D_shared, C[pid_b, pid_n * BLOCK_Q, pid_m * BLOCK_Q])
            else:
...
def _quad_bq(
    ...
    def _quad_bq_(
        A: T.Tensor((B, M, M), dtype),
        A_scale: T.Tensor((B, md, md), 'float32'),
        A_diag: T.Tensor((B, M), 'float32'),
        C: T.Tensor((B, M, M), dtype),
        C_scale: T.Tensor((B, md, md), 'float32'),
        C_diag: T.Tensor((B, M), 'float32'),
        C0: T.float32,
        ...
        with T.Kernel(B, total_blocks, threads=threads) as (pid_b, pid):
            ...
            T.copy(A_diag[pid_b, pid_m * BLOCK_Q], A_diag_shared)
            T.copy(A_diag[pid_b, pid_n * BLOCK_Q], B_diag_shared)
            A_shared_2 = T.alloc_fragment((BLOCK_Q, BLOCK_Q), dtype)
            A_scale_1 = A_scale[pid_b, pid_m, pid_n]
            T.copy(A[pid_b, pid_m * BLOCK_Q, pid_n * BLOCK_Q], A_shared_2)
            is_diag = T.alloc_var(T.bool)
            is_diag = pid_m == pid_n
            ...
            for i, j in T.Parallel(BLOCK_Q, BLOCK_Q):
                ...
                if is_diag and i == j:
                    C_diag[pid_b, pid_m * BLOCK_Q + i] = C_float[i, j] + (C0 + ai * (C1 + ai * C2))
                    C_float[i, j] = 0
                else:
                    C_scale_2[0] = T.max(C_scale_2[0], T.abs(C_float[i, j]))
            T.finalize_reducer(C_scale_2)
...
def _typeii_typei_bq(
    ...
    def _typeii_typei_bq_(
        A:       T.Tensor((B, M, M), dtype),
        A_scale: T.Tensor((B, md, md), T.float32),
        A_diag:  T.Tensor((B, M), T.float32),
        B_mat:   T.Tensor((B, M, N), dtype),
        B_scale: T.Tensor((B, md, nd), T.float32),
        C:       T.Tensor((B, M, N), dtype),
        C_scale: T.Tensor((B, md, nd), T.float32),
    ):
        with T.Kernel(B, nd, md, threads=threads) as (pid_b, pid_n, pid_m):
            A_shared = T.alloc_shared((BLOCK_Q, BLOCK_Q), dtype)
            B_shared = T.alloc_shared((BLOCK_Q, BLOCK_Q), dtype)
            ...
            for k in T.Pipelined(md, num_stages=num_stages):
                T.clear(C_local)
                T.copy(A[pid_b, pid_m * BLOCK_Q, k * BLOCK_Q], A_shared)
                T.copy(B_mat[pid_b, k * BLOCK_Q, pid_n * BLOCK_Q], B_shared)
                T.gemm(A_shared, B_shared, C_local)
                A_scale_1 = A_scale[pid_b, pid_m, k] * B_scale[pid_b, k, pid_n]
                for i, j in T.Parallel(BLOCK_Q, BLOCK_Q):
                    C_float[i, j] = T.cast(C_local[i, j], T.float32) * A_scale_1 + C_float[i, j]

            B8 = T.alloc_fragment((BLOCK_Q, BLOCK_Q), dtype)
            T.copy(A_diag[pid_b, pid_m * BLOCK_Q], row_shared)
            T.copy(B_mat[pid_b, pid_m * BLOCK_Q, pid_n * BLOCK_Q], B8)
            B_var = T.alloc_var(T.float32)
            B_var = B_scale[pid_b, pid_m, pid_n]
            Cmax_reducer = T.alloc_reducer((1,), T.float32, op='max', replication="all")
            T.fill(Cmax_reducer, 0)
            for (i, j) in T.Parallel(BLOCK_Q, BLOCK_Q):
...
def _to_bq_transpose_out(
    ...
    DTYPE_MAX = 448 if 'float8_e4m3' in dtype_out else 127 
    @T.prim_func
    def _to_bq_transpose_out_(
        A: T.Tensor((B, M, N), dtype),
        A_sumsq_or_norm: T.Tensor((B,), dtype2),
        A_bq: T.Tensor((B, N, M), dtype_out),
        A_scale: T.Tensor((B, nd, md), T.float32)
    ):
        with T.Kernel(B, nd, md, threads=threads) as (pid_b, pid_n, pid_m):
            A_scale_1 = T.alloc_var(T.float32)
            A_scale_2 = T.alloc_reducer((1,), T.float32, op="max", replication="all")
            ...
            A_scale_1 = DTYPE_MAX / A_scale_2[0]
            if (T.get_lane_idx() == 0 and T.get_warp_idx() == 0):
                A_scale[pid_b, pid_n, pid_m] = 1 / A_scale_1
            for i, j in T.Parallel(BLOCK_Q, BLOCK_Q):
                A_local[i, j] = A_local[i, j] * A_scale_1
            for j, i in T.Parallel(BLOCK_Q, BLOCK_Q):
                A_bq[pid_b, pid_n * BLOCK_Q + j, pid_m * BLOCK_Q + i] = T.cast(A_local[i, j], dtype_out)
    return _to_bq_transpose_out_


def _typeii_typei_final_bq_transpose_out(
    B: int = 1,
    M: int = 4096, N: int= 6144,
    threads: int = 256, num_stages: int = 2,
    dtype: str = 'float8_e4m3fn',
    ...
    @T.prim_func
    def _typeii_typei_final_bq_transpose_out_(
        A:       T.Tensor((B, M, M), dtype),
        A_scale: T.Tensor((B, md, md), T.float32),
        A_diag:  T.Tensor((B, M), T.float32),
        B_mat:   T.Tensor((B, M, N), dtype),
        B_scale: T.Tensor((B, md, nd), T.float32),
        C:       T.Tensor((B, N, M), dtype2),
    ):
        with T.Kernel(B, nd, md, threads=threads) as (pid_b, pid_n, pid_m):
            A_shared = T.alloc_shared((BLOCK_Q, BLOCK_Q), dtype)
            B_shared = T.alloc_shared((BLOCK_Q, BLOCK_Q), dtype)
            ...
            for j, i in T.Parallel(BLOCK_Q, BLOCK_Q):
                C[pid_b, pid_n * BLOCK_Q + j, pid_m * BLOCK_Q + i] = C_float[i, j]
    return _typeii_typei_final_bq_transpose_out_


####################### Fused transpose-out variants ########################################

def _scale_int8_transpose_out(
    B: int = 1,
    M: int = 4096, N: int=6144, 
    BLOCK_M: int = 8, BLOCK_N: int = 64, 
    threads: int = 256,
    ...
    @T.prim_func
    def _scale_int8_transpose_out_(
        A: T.Tensor((B, M, N), dtype),
        A_max: T.Tensor((B,), T.float32),
        A_sumsq_or_norm: T.Tensor((B,), dtype2),
        A_int8: T.Tensor((B, N, M), T.int8),
        A_scale: T.Tensor((B,), T.float32)
    ):
        with T.Kernel(B, T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (pid_b, pid_n, pid_m):
            if pid_n == 0 and pid_m == 0 and T.get_lane_idx() == 0 and T.get_warp_idx() == 0:
            ...
            for i, j in T.Parallel(BLOCK_M, BLOCK_N):
                C_local_i8[i, j] = T.cast(T.round((127.0 / A_max[pid_b]) * T.cast(C_local[i, j], T.float32)), T.int8)
            A_shared = T.alloc_shared((BLOCK_M, BLOCK_N), T.int8)
            A_shared_T = T.alloc_shared((BLOCK_N, BLOCK_M), T.int8)
            T.copy(C_local_i8, A_shared)
            T.transpose(A_shared, A_shared_T)
            T.copy(A_shared_T, A_int8[pid_b, pid_n * BLOCK_N, pid_m * BLOCK_M])
    return _scale_int8_transpose_out_


def _typeii_typei_int8_transpose_out(
    B: int = 1,
    M: int = 4096, N: int= 6144,
    BLOCK_M: int = 256, BLOCK_N: int = 128, BLOCK_K: int = 128,
    threads: int = 256, num_stages: int = 2
    ...
    @T.prim_func
    def _typeii_typei_int8_transpose_out_(
        A: T.Tensor((B, M, M), T.int8),
        A_scale: T.Tensor((B,), T.float32),
        A_diag: T.Tensor((B, M), T.float32),
        B_mat: T.Tensor((B, M, N), T.int8),
        B_scale: T.Tensor((B,), T.float32),
        C: T.Tensor((B, N, M), T.float32),
        C_max: T.Tensor((B,), T.float32),
    ):
        with T.Kernel(B, T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (pid_b, pid_n, pid_m):
            A_shared = T.alloc_shared((BLOCK_M, BLOCK_K), T.int8)
...
def _to_prec_transpose_out(
    ...
    @T.prim_func
    def _to_prec_transpose_out_(
        A: T.Tensor((B, M, N), dtype),
        A_sumsq_or_norm: T.Tensor((B,), dtype2),
        A_prec: T.Tensor((B, N, M), dtype_out),
    ):
        with T.Kernel(B, T.ceildiv(N, BLOCK_N), T.ceildiv(M, BLOCK_M), threads=threads) as (pid_b, pid_n, pid_m):
            A_scale = T.alloc_var(T.float32)
            A_scale = 1.0 / T.max(
            ...
            A_shared_T = T.alloc_shared((BLOCK_N, BLOCK_M), T.float32)
            T.copy(A_local, A_shared)
            T.transpose(A_shared, A_shared_T)
            T.copy(A_shared_T, A_prec[pid_b, pid_n * BLOCK_N, pid_m * BLOCK_M])
    return _to_prec_transpose_out_
...
def _ab_symm_bq(
    ...
    def _ab_symm_bq_(
        ...
        A_diag: T.Tensor((B, M), 'float32'),
        B_mat: T.Tensor((B, M, M), dtype),
        B_scale: T.Tensor((B, md, md), 'float32'),
        B_diag: T.Tensor((B, M), 'float32'),
        C: T.Tensor((B, M, M), dtype),
        C_scale: T.Tensor((B, md, md), 'float32'),
        C_diag: T.Tensor((B, M), 'float32'),
    ):
        with T.Kernel(B, total_blocks, threads=threads) as (pid_b, pid):
            ...
            for k in T.Pipelined(md, num_stages=num_stages):
                T.clear(C_local)
                T.copy(A[pid_b, pid_m * BLOCK_Q, k * BLOCK_Q], A_shared)
                T.copy(B_mat[pid_b, pid_n * BLOCK_Q, k * BLOCK_Q], B_shared)
                T.gemm(A_shared, B_shared, C_local, transpose_B=True)
                A_scale_1 = A_scale[pid_b, pid_m, k] * B_scale[pid_b, pid_n, k]
                for i, j in T.Parallel(BLOCK_Q, BLOCK_Q):
                    C_float[i, j] = T.cast(C_local[i, j], T.float32) * A_scale_1 + C_float[i, j] 

            T.copy(A_diag[pid_b, pid_m * BLOCK_Q], A_diag_shared)
            T.copy(B_diag[pid_b, pid_n * BLOCK_Q], B_diag_shared)
            A_shared_2 = T.alloc_fragment((BLOCK_Q, BLOCK_Q), dtype)
            B_shared_2 = T.alloc_fragment((BLOCK_Q, BLOCK_Q), dtype)
            A_scale_1 = A_scale[pid_b, pid_m, pid_n]
            B_scale_1 = B_scale[pid_b, pid_m, pid_n]
            T.copy(A[pid_b, pid_m * BLOCK_Q, pid_n * BLOCK_Q], A_shared_2)
            T.copy(B_mat[pid_b, pid_m * BLOCK_Q, pid_n * BLOCK_Q], B_shared_2)
            ...
            for i, j in T.Parallel(BLOCK_Q, BLOCK_Q):
                ...
                if is_diag and i == j:
                    C_diag[pid_b, pid_m * BLOCK_Q + i] = C_float[i, j] + A_diag_shared[i] * B_diag_shared[j]
                    C_float[i, j] = 0
                else:
                    C_scale_2[0] = T.max(C_scale_2[0], T.abs(C_float[i, j]))
            T.finalize_reducer(C_scale_2)
...
__all__ = [
    # global int8
    '_sumsq_maxabs', '_scale_int8', '_scale_int8_transpose_out',
    '_aat_int8_max', '_int32_compl_symm_int8', '_typeii_int8_sq',
    '_float32_compl_symm_int8_quad', '_typeii_int8_ab', '_float32_ab_to_int8',
    '_typeii_typei_int8', '_typeii_typei_int8_transpose_out',
    '_float32_to_int8', 
    # fp16/bf16/fp32
    '_to_prec', '_to_prec_transpose_out', '_ab_prec', '_ab_prec_transpose_out',
    '_aat_prec', '_quad_prec', '_ab_symm_prec', 
    # block fp8/int8
    '_to_bq', '_to_bq_transpose_out', '_aat_bq','_quad_bq', '_typeii_typei_bq', 
    '_typeii_typei_final_bq', '_typeii_typei_final_bq_transpose_out', '_ab_symm_bq',
    'BLOCK_Q'
]
